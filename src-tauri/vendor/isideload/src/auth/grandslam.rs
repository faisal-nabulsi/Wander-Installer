...
impl GrandSlam {
    ...
    pub fn build_reqwest_client(debug: bool) -> Result<reqwest::Client, Report> {
        ...
        let client = ClientBuilder::new()
            ...
            // Wander patch (mirrors upstream nab138/isideload f6a4d5d): Apple's edge in front of
            // gsa.apple.com serves at most two requests per connection and answers the rest with
            // HTTP 429. Signing in is url-bag GET -> SRP init -> SRP proof, so on a pooled
            // connection the proof was always the third request and always failed. No idle
            // pooling means every request gets a fresh connection.
